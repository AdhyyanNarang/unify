...
def train(sess, env, X_data, y_data, X_valid=None, y_valid=None, epochs=1,
    ...
    if hasattr(env, 'saver'):
        print('\n Saving model')
        os.makedirs('model_37', exist_ok=True)
        env.saver.save(sess, 'model_37/{}'.format(name))
...
print('\nStarting adversarial training process')

print('\nSplitting data into benign and adversarial fractions')

ADV_SPLIT = 0.4

print('\nFraction of adversarial examples in train set:' + str(ADV_SPLIT))
thres = int(X_train.shape[0] * (1-ADV_SPLIT))
X_adv_portion = X_train[thres:]
X_benign = X_train[:thres]

y_selected = y_train[thres:]
matplotlib.pyplot.hist(y_selected, range(10))
os.makedirs('img', exist_ok=True)
plt.savefig('img/adhyyan.png')

## normal training
print('normal training')
train(sess, env, X_train, y_train, X_valid, y_valid, load=False, epochs=2,
      name='mnist')
print('\nGenerating adversarial data')

X_adv_test = make_fgsm(sess, env, X_test, eps=0.02, epochs=12)
# X_adv = make_deepfool(sess,env,X_test,epochs=3)
...
print('\nEvaluating on adversarial data')
evaluate(sess, env, X_adv_test, y_test)



print('adversarial training')
X_adv = make_fgsm(sess, env, X_adv_portion, eps=0.02, epochs=12)
X_adv_train = np.vstack((X_benign, X_adv))
print('\nSize of dataset:' + str(len(X_adv_train)))
sess.run(tf.global_variables_initializer())
sess.run(tf.local_variables_initializer())
train(sess, env, X_adv_train, y_train, X_valid, y_valid, load=False, epochs=2,
      name='mnist')
