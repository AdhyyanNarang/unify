...
def console_log(msg, end='\n'):
    os.write(1, ('[LOG/{}]'.format(multiprocessing.current_process())+msg+end).encode('utf-8'))
...
# def data_labels_distances_mapping_img(x,classifier_fn,num_samples):
#     distance_fn = lambda x: sklearn.metrics.pairwise.cosine_distances(x[0], x)[0] * 100

def data_labels_distances_mapping_text(x,
    classifier_fn, num_samples, distance_metric='cosine',shape=(28,28,1)):
        # distance_fn = lambda x : sklearn.metrics.pairwise.cosine_distances(
                # x,x[0,:].reshape(1,-1)
            # )[0] * 100 
        def distance_fn(x):
        ...
        features = x.nonzero()[1] # indices for nonzero elements (don't care about [0], because x is an array and [0] are the same)
        vals = np.array(x[x.nonzero()])
        doc_size = len(sp.sparse.find(x)[2])    # number of nonzero features
        sample = np.random.randint(1, doc_size, num_samples - 1)    # the number of nonzero pixels have to be smaller than the central point
        data = np.zeros((num_samples, len(features)))    
        inverse_data = np.zeros((num_samples, len(features)))                                         
        data[0] = np.ones(doc_size) # the first row is the original feature
        inverse_data[0] = vals
        ...
        for i, s in enumerate(sample, start=1):
                ...
                # data[i, active] = 1
                data[i,active] = vals[active]
                for j in active:
                        inverse_data[i, j] = vals[j]
        ...
        mapping = features # the index of nonzero pixels
        dense_inverse = np.array(sparse_inverse.todense())
        #print(dense_inverse.shape)
        # pdb.set_trace()
        dense_inverse_reshape = dense_inverse.reshape((dense_inverse.shape[0],*shape))
        labels = classifier_fn(dense_inverse_reshape)
        console_log(' computing dist')
        distances = distance_fn(dense_inverse)
        console_log(' computing dist/done')
        # labels = classifier_fn(sparse_inverse)
        # distances = distance_fn(sparse_inverse)
        return data, labels, distances, mapping, dense_inverse_reshape



def data_labels_distances_mapping_identity(x,
    classifier_fn, num_samples, shape=(28,28,1)): 
        # distance_fn = lambda x : sklearn.metrics.pairwise.cosine_distances(
                # x,x[0,:].reshape(1,-1)
            # )[0] * 100 
        def distance_fn(x):
...
class GeneralizedLocalExplainer:
    ...
    def data_labels_distances_mapping(self, raw_data, classifier_fn):
        data, labels, distances, mapping, dense_inverse_reshape = self.data_labels_distances_mapping_fn(raw_data, classifier_fn, self.num_samples)
        return data, labels, distances, mapping, dense_inverse_reshape
    ...
    def explain_instance_with_data(self, data, labels, distances, label, num_features):
        ...
        debiased_model = linear_model.Ridge(alpha=0, fit_intercept=False) # normal linear regression without regulairization
    ...
    def explain_instance(self,
        ...
        data, labels, distances, mapping, dense_inverse_reshape = self.data_labels_distances_mapping(raw_data, classifier_fn)
        if self.return_mapped:
            ...
            if self.return_mean:
                return exp, mean, dense_inverse_reshape, labels
            else:
                return exp, dense_inverse_reshape, labels
        return self.explain_instance_with_data(data, labels, distances, label, num_features), mapping, dense_inverse_reshape, labels
